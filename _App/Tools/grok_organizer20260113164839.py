import os
import shutil
import glob
import json
import html
import re
import webbrowser
from pathlib import Path
from datetime import datetime

# 画像処理ライブラリ
try:
    from PIL import Image
except ImportError:
    Image = None

# ==========================================
# 設定エリア
# ==========================================

DOWNLOAD_DIR = Path(os.path.expanduser("~")) / "Downloads"

# スクリプトがある場所（Organizer）の1つ上をルートディレクトリとする
# 構成: Downloads/Grok-Auto-Saver/Organizer/grok_organizer.py
# ルート: Downloads/Grok-Auto-Saver/
BASE_DIR = Path(__file__).parent.resolve()

# 安全策: もしOrganizerフォルダに入っていない場合は、現在のフォルダをルートとする
if BASE_DIR.name.lower() == "organizer":
    GROK_ROOT_DIR = BASE_DIR.parent
else:
    GROK_ROOT_DIR = BASE_DIR

# フォルダパス定義
DEST_DIR = GROK_ROOT_DIR / "Favorites"
VIEWER_PATH = GROK_ROOT_DIR / "Grok_Viewer.html"
MERGED_PROMPT_FILE = "All_Prompts_Merged.txt"
FAVORITES_DB_FILE = "All_Favorites_Merged.json"

# ==========================================
# 処理ロジック
# ==========================================

def is_safe_directory(path):
    """
    削除処理を行っても良い安全なフォルダかチェックする
    Downloads直下やユーザーホーム直下での実行を防ぐ
    """
    user_home = Path(os.path.expanduser("~"))
    unsafe_paths = [
        DOWNLOAD_DIR,
        user_home,
        user_home / "Desktop",
        user_home / "Documents"
    ]
    
    # パスが一致してしまったらNG
    if path.resolve() in [p.resolve() for p in unsafe_paths if p.exists()]:
        return False
        
    return True

def move_videos():
    """動画ファイルを日付フォルダへ移動し、ファイル名に日時を付与する"""
    print(f"🎬 動画移動処理開始...")
    
    count = 0
    target_files = list(DOWNLOAD_DIR.glob("grok-video-*.mp4"))

    if not target_files:
        print("   動画ファイルは見つかりませんでした。")
        return 0

    for file_path in target_files:
        try:
            # ファイルの更新日時から日付フォルダ名 (YYYYMMDD) を決定
            ts = file_path.stat().st_mtime
            dt = datetime.fromtimestamp(ts)
            date_str = dt.strftime('%Y%m%d')
            # ルートディレクトリ配下の日付フォルダへ
            target_dir = GROK_ROOT_DIR / date_str

            if not target_dir.exists():
                try:
                    target_dir.mkdir(parents=True, exist_ok=True)
                except Exception:
                    pass 

            # ★ファイル名に日時を付与してリネーム (画像と同じフォーマットでソート可能にする)
            if not re.search(r'_\d{8}_\d{6}_', file_path.name):
                time_str = dt.strftime('%Y%m%d_%H%M%S')
                new_name = file_path.name.replace("grok-video-", f"grok-video_{time_str}_")
                if new_name == file_path.name:
                    new_name = f"{time_str}_{file_path.name}"
            else:
                new_name = file_path.name

            target_path = target_dir / new_name
            
            # 同名ファイル回避
            if target_path.exists():
                stem = Path(new_name).stem
                suffix = Path(new_name).suffix
                counter = 1
                while target_path.exists():
                    target_path = target_dir / f"{stem}_{counter}{suffix}"
                    counter += 1
            
            shutil.move(str(file_path), str(target_path))
            print(f"   ✅ [移動] {file_path.name} -> {date_str}/{target_path.name}")
            count += 1
        except Exception as e:
            print(f"   ❌ [エラー] {file_path.name}: {e}")
            
    return count

def clean_garbage_images():
    """Grokフォルダ配下の全フォルダから不要な画像を削除する"""
    print(f"\n🖼️ 画像クリーニング処理開始...")
    
    # 安全装置発動
    if not is_safe_directory(GROK_ROOT_DIR):
        print(f"   ⚠️ 警告: 安全装置が作動しました。")
        print(f"   現在のフォルダ ({GROK_ROOT_DIR}) は削除処理に適していません。")
        print(f"   スクリプトを専用のサブフォルダ（例: Grok-Auto-Saver）の中に移動してください。")
        return 0

    if Image is None:
        print("   ⚠️  Pillowライブラリがありません。スキップします。")
        return 0

    if not GROK_ROOT_DIR.exists():
        return 0
    
    count = 0
    print("   画像をスキャン中...", end="", flush=True)
    all_image_files = []
    for ext in ["*.png", "*.jpg", "*.jpeg", "*.webp"]:
        all_image_files.extend(list(GROK_ROOT_DIR.rglob(ext)))
    print(f" 完了 ({len(all_image_files)} ファイル)")

    for i, file_path in enumerate(all_image_files):
        if i % 100 == 0:
             print(f"\r   処理中: {i}/{len(all_image_files)}...", end="", flush=True)

        try:
            # 自分のアイコンや素材フォルダなどは除外
            if "icons" in file_path.parts: continue
            if "ChromeExtension" in file_path.parts: continue

            should_remove = False
            reason = ""

            if file_path.name.endswith("profile-picture.webp"):
                should_remove = True
                reason = "User Profile Picture"
            else:
                with Image.open(file_path) as img:
                    width, height = img.size
                    if img.mode in ('RGBA', 'CMYK'):
                        should_remove = True
                        reason = f"Mode: {img.mode}"
                    elif min(width, height) < 500:
                        should_remove = True
                        reason = f"Small Size: {width}x{height}"

            if should_remove:
                print(f"\r   🗑️ [削除] {file_path.name} ({reason}){' '*20}") 
                os.remove(file_path)
                count += 1
                
        except Exception:
            pass
            
    print(f"\r   処理完了{' '*30}")
    return count

def organize_prompts():
    """プロンプトのマージ"""
    print(f"\n📝 プロンプト整理処理開始...")
    prompts_dir = GROK_ROOT_DIR / "Prompts"
    if not prompts_dir.exists(): return 0

    archive_dir = prompts_dir / "Archived"
    archive_dir.mkdir(exist_ok=True)

    txt_files = list(prompts_dir.glob("*.txt"))
    unique_prompts = {}
    prompt_pattern = re.compile(r'^\[(\d{4}/\d{1,2}/\d{1,2} \d{1,2}:\d{2}:\d{2})\]\s*\n(.*?)(?=\n-{20,}|\Z)', re.DOTALL | re.MULTILINE)
    files_to_archive = []

    print(f"   {len(txt_files)} 個のファイルを処理中...")

    for txt_path in txt_files:
        is_merged_file = (txt_path.name == MERGED_PROMPT_FILE)
        try:
            with open(txt_path, "r", encoding="utf-8-sig") as f:
                content = f.read()
            found = False
            for match in prompt_pattern.finditer(content):
                found = True
                dt_str = match.group(1)
                text = match.group(2).strip()
                try:
                    dt = datetime.strptime(dt_str, '%Y/%m/%d %H:%M:%S')
                    ts = dt.timestamp()
                    if text in unique_prompts:
                        if ts > unique_prompts[text]['time']:
                            unique_prompts[text] = {'time': ts, 'date_str': dt_str, 'content': text}
                    else:
                        unique_prompts[text] = {'time': ts, 'date_str': dt_str, 'content': text}
                except ValueError: continue
            
            if not is_merged_file and found:
                files_to_archive.append(txt_path)
        except Exception: pass

    if unique_prompts:
        sorted_prompts = sorted(unique_prompts.values(), key=lambda x: x['time'], reverse=True)
        merged_path = prompts_dir / MERGED_PROMPT_FILE
        try:
            with open(merged_path, "w", encoding="utf-8") as f:
                f.write("GrokSaver Prompt History (Merged)\n====================================\n\n")
                for item in sorted_prompts:
                    f.write(f"[{item['date_str']}]\n{item['content']}\n------------------------------------\n\n")
            
            for src in files_to_archive:
                try:
                    dst = archive_dir / src.name
                    if dst.exists():
                        ts = datetime.now().strftime("%Y%m%d%H%M%S")
                        dst = archive_dir / f"{src.stem}_{ts}{src.suffix}"
                    shutil.move(str(src), str(dst))
                except Exception: pass
        except Exception: pass

def organize_favorites():
    """Favoritesログの統合"""
    print(f"\n⭐ Favoritesログ整理処理開始...")
    logs_dir = GROK_ROOT_DIR / "System" / "FavLogs"
    system_dir = GROK_ROOT_DIR / "System"
    
    if not logs_dir.exists(): return set()

    archive_dir = logs_dir / "Archived"
    archive_dir.mkdir(parents=True, exist_ok=True)

    merged_db_path = system_dir / FAVORITES_DB_FILE
    favorites_set = set()
    
    if merged_db_path.exists():
        try:
            with open(merged_db_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                for item in data: favorites_set.add(item['filename'])
        except Exception: pass

    log_files = list(logs_dir.glob("*.json"))
    new_fav_count = 0
    files_to_archive = []

    print(f"   {len(log_files)} 個のログファイルを処理中...")

    for log_file in log_files:
        try:
            with open(log_file, "r", encoding="utf-8") as f:
                items = json.load(f)
                if isinstance(items, list):
                    for item in items:
                        if item.get('filename'):
                            favorites_set.add(item['filename'])
                            new_fav_count += 1
                elif isinstance(items, dict) and items.get('filename'):
                    favorites_set.add(items['filename'])
                    new_fav_count += 1
            files_to_archive.append(log_file)
        except Exception: pass

    if new_fav_count > 0 or not merged_db_path.exists():
        try:
            save_list = [{"filename": name} for name in favorites_set]
            if not system_dir.exists(): system_dir.mkdir(parents=True, exist_ok=True)
            with open(merged_db_path, "w", encoding="utf-8") as f:
                json.dump(save_list, f, indent=2, ensure_ascii=False)
            
            for src in files_to_archive:
                try:
                    dst = archive_dir / src.name
                    if dst.exists():
                        ts = datetime.now().strftime("%Y%m%d%H%M%S")
                        dst = archive_dir / f"{src.stem}_{ts}{src.suffix}"
                    shutil.move(str(src), str(dst))
                except Exception: pass
        except Exception: pass

    return favorites_set

# ==========================================
# ビューアー生成
# ==========================================

def get_file_info(path, type_label, fav_set):
    ts = path.stat().st_mtime
    match = re.search(r'_(\d{8})_(\d{6})_', path.name)
    if match:
        try:
            dt_str = match.group(1) + match.group(2)
            dt = datetime.strptime(dt_str, '%Y%m%dH%M%S')
            ts = dt.timestamp()
        except ValueError: pass
    
    is_favorite = path.name in fav_set
    
    return {
        'type': type_label,
        'name': path.name,
        'time': ts,
        'date_str': datetime.fromtimestamp(ts).strftime('%Y-%m-%d'), 
        'path': os.path.relpath(path, GROK_ROOT_DIR).replace("\\", "/"),
        'is_favorite': is_favorite
    }

def collect_and_group_data(fav_set):
    if not GROK_ROOT_DIR.exists(): return {}
    all_items = []

    print("   ビューアー用データを収集中...", end="", flush=True)
    media_files = []
    for ext in ["*.png", "*.jpg", "*.jpeg", "*.webp", "*.mp4"]:
        media_files.extend(list(GROK_ROOT_DIR.rglob(ext)))
    print(f" 対象: {len(media_files)}件")

    for i, path in enumerate(media_files):
        if i % 500 == 0:
            print(f"\r   インデックス作成中: {i}/{len(media_files)}...", end="", flush=True)

        try:
            # Organizer, System, Prompts, ChromeExtension フォルダは除外
            if path.parent.name in ["Prompts", "System", "Organizer", "ChromeExtension", "icons"]: continue
            if "Organizer" in path.parts: continue

            type_label = 'video' if path.suffix.lower() == '.mp4' else 'image'
            all_items.append(get_file_info(path, type_label, fav_set))
        except Exception: continue
    
    print(f"\r   インデックス作成完了{' '*20}")

    merged_file = GROK_ROOT_DIR / "Prompts" / MERGED_PROMPT_FILE
    if merged_file.exists():
        print("   プロンプト履歴を解析中...")
        prompt_pattern = re.compile(r'^\[(\d{4}/\d{1,2}/\d{1,2} \d{1,2}:\d{2}:\d{2})\]\s*\n(.*?)(?=\n-{20,}|\Z)', re.DOTALL | re.MULTILINE)
        try:
            with open(merged_file, "r", encoding="utf-8") as f:
                content = f.read()
            for match in prompt_pattern.finditer(content):
                dt_str, text = match.group(1), match.group(2).strip()
                if not text: continue
                try:
                    dt = datetime.strptime(dt_str, '%Y/%m/%d %H:%M:%S')
                    all_items.append({
                        'type': 'prompt', 'name': "History", 'time': dt.timestamp(),
                        'date_str': dt.strftime('%Y-%m-%d'), 'content': text, 'is_favorite': False
                    })
                except ValueError: continue
        except Exception: pass

    date_buckets = {}
    for item in all_items:
        date_key = item['date_str']
        if date_key not in date_buckets: date_buckets[date_key] = []
        date_buckets[date_key].append(item)

    timeline_data = {}
    for date_key, items in date_buckets.items():
        items.sort(key=lambda x: x['time'], reverse=True)
        grouped_list = []
        current_media_group = []

        for item in items:
            if item['type'] in ('image', 'video'):
                current_media_group.append(item)
            elif item['type'] == 'prompt':
                if current_media_group:
                    grouped_list.append({"prompt": item, "media": current_media_group})
                    current_media_group = []
        
        if current_media_group:
            grouped_list.append({"prompt": None, "media": current_media_group})
        
        if grouped_list: timeline_data[date_key] = grouped_list

    return timeline_data

def generate_viewer_html(fav_set):
    print(f"\n🌐 ビューアー生成処理開始...")
    data = collect_and_group_data(fav_set)
    if not data: return

    dates = sorted(list(data.keys()), reverse=True)
    json_data = json.dumps(data, ensure_ascii=False)
    dates_json = json.dumps(dates)

    html_content = f"""<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Grok Viewer</title>
    <style>
        :root {{ --bg: #121212; --sidebar-bg: #1e1e1e; --card-bg: #252525; --text: #e0e0e0; --accent: #bb86fc; --border: #333; }}
        body {{ font-family: 'Segoe UI', sans-serif; background: var(--bg); color: var(--text); margin: 0; display: flex; height: 100vh; overflow: hidden; }}
        
        #sidebar {{ width: 220px; background: var(--sidebar-bg); border-right: 1px solid var(--border); display: flex; flex-direction: column; flex-shrink: 0; }}
        .sidebar-header {{ 
            padding: 15px; 
            font-weight: bold; 
            border-bottom: 1px solid var(--border); 
            font-size: 1.1rem; 
            display: flex; 
            justify-content: space-between; 
            align-items: center; 
            white-space: nowrap; 
        }}
        #search-toggle {{ 
            cursor: pointer; 
            font-size: 1.2rem; 
            opacity: 0.7; 
            transition: all 0.2s; 
            width: 30px; 
            height: 30px; 
            display: flex; 
            justify-content: center; 
            align-items: center; 
            border-radius: 50%;
            margin-left: 10px; 
        }}
        #search-toggle:hover {{ opacity: 1; color: var(--accent); background: rgba(255,255,255,0.1); }}

        #normal-sidebar-content {{ display: flex; flex-direction: column; flex: 1; overflow: hidden; }}
        #search-sidebar-content {{ display: flex; flex-direction: column; flex: 1; overflow: hidden; padding: 10px; }}
        #search-sidebar-content.hidden {{ display: none !important; }}
        #normal-sidebar-content.hidden {{ display: none !important; }}

        .search-box {{ display: flex; gap: 5px; margin-bottom: 10px; }}
        #search-input {{ flex: 1; padding: 8px; border-radius: 4px; border: 1px solid #444; background: #333; color: white; min-width: 0; }} 
        #search-input:focus {{ outline: 2px solid var(--accent); }}
        .search-btn {{ 
            padding: 0;
            width: 36px;
            height: 36px;
            background: var(--accent); 
            border: none; 
            border-radius: 4px; 
            color: #000; 
            font-weight: bold; 
            cursor: pointer; 
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 1.2rem;
        }}
        .back-btn {{ margin-top: auto; padding: 10px; background: none; border: 1px solid #555; color: #ccc; cursor: pointer; border-radius: 4px; }}
        .back-btn:hover {{ background: #333; }}
        #search-status {{ font-size: 0.9rem; color: #aaa; margin-bottom: 15px; }}

        .filter-tabs {{ display: flex; border-bottom: 1px solid var(--border); }}
        .filter-tab {{ flex: 1; padding: 10px; text-align: center; cursor: pointer; background: rgba(0,0,0,0.2); transition: background 0.2s; font-size: 0.9rem; }}
        .filter-tab:hover {{ background: rgba(255,255,255,0.05); }}
        .filter-tab.active {{ background: var(--accent); color: #000; font-weight: bold; }}
        #date-list {{ list-style: none; padding: 0; margin: 0; overflow-y: auto; flex: 1; }}
        .date-item {{ padding: 10px 15px; cursor: pointer; border-bottom: 1px solid #2a2a2a; transition: background 0.2s; display: flex; justify-content: space-between; }}
        .date-item:hover {{ background: #2a2a2a; }}
        .date-item.active {{ background: rgba(187, 134, 252, 0.2); border-left: 4px solid var(--accent); }}
        .date-item.hidden {{ display: none; }}
        .count {{ font-size: 0.8rem; opacity: 0.6; background: rgba(0,0,0,0.3); padding: 2px 6px; border-radius: 10px; }}
        
        /* メインコンテンツ */
        #main {{ 
            flex: 1; 
            overflow-y: scroll; 
            scrollbar-gutter: stable; 
            padding: 20px; 
            position: relative; 
            scroll-behavior: smooth; 
        }}
        
        .group {{ margin-bottom: 40px; background: var(--card-bg); border-radius: 8px; padding: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.3); }}
        .prompt-header {{ margin-bottom: 15px; padding-bottom: 10px; border-bottom: 1px solid var(--border); }}
        .prompt-meta {{ font-size: 0.85rem; color: #888; margin-bottom: 5px; }}
        .prompt-text {{ white-space: pre-wrap; font-size: 1rem; line-height: 1.6; color: #fff; }}
        .no-prompt {{ color: #666; font-style: italic; font-size: 0.9rem; padding-bottom: 10px; }}
        
        /* Media Grid */
        .media-grid {{ 
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(200px, 1fr));
            gap: 10px;
            align-items: start;
        }}
        .media-item {{ 
            background: #000; 
            cursor: pointer; 
            border-radius: 4px; 
            overflow: hidden;
            position: relative;
        }}
        .media-item img, .media-item video {{ 
            width: 100%; 
            height: auto; 
            display: block; 
            transition: transform 0.2s; 
        }}
        .media-item:hover img {{ transform: scale(1.02); }}
        .media-item.hidden {{ display: none; }}
        .video-label {{ position: absolute; top: 5px; right: 5px; background: rgba(0,0,0,0.7); color: white; padding: 2px 6px; border-radius: 4px; font-size: 0.7rem; pointer-events: none; }}
        
        /* Modal & Image Fit */
        #modal {{ display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 1000; justify-content: center; align-items: center; }}
        #modal.active {{ display: flex; }}
        #modal-content {{ position: relative; max-width: 95%; max-height: 95%; display: flex; justify-content: center; align-items: center; width: 100%; height: 100%; }}
        
        /* ★修正★ モーダル内の画像・動画は画面サイズに合わせてフィットさせる */
        #modal-content img, #modal-content video {{ 
            width: 100%; 
            height: 95vh; 
            object-fit: contain; 
            box-shadow: 0 0 20px rgba(0,0,0,0.5); 
        }}
        
        .close-hint {{ position: absolute; top: 20px; right: 20px; color: white; background: rgba(0,0,0,0.5); padding: 5px 10px; border-radius: 5px; cursor: pointer; z-index: 1002; }}
        
        .modal-nav {{ position: absolute; top: 50%; transform: translateY(-50%); background: rgba(0,0,0,0.5); color: white; border: none; font-size: 2rem; padding: 20px 10px; cursor: pointer; z-index: 1001; transition: background 0.2s; }}
        .modal-nav:hover {{ background: rgba(187, 134, 252, 0.8); color: black; }}
        .modal-prev {{ left: 20px; }}
        .modal-next {{ right: 20px; }}
        
        /* Navigation Buttons */
        #nav-buttons {{ 
            padding: 10px; 
            border-top: 1px solid var(--border); 
            display: grid; 
            grid-template-columns: 1fr 1fr; 
            gap: 5px; 
            background: var(--sidebar-bg); 
        }}
        .nav-btn {{ 
            width: 100%; 
            height: 35px; 
            background: rgba(50, 50, 50, 0.5); 
            color: #ccc; 
            border: 1px solid #444; 
            border-radius: 4px; 
            cursor: pointer; 
            font-size: 1.1rem; 
            display: flex; 
            justify-content: center; 
            align-items: center; 
            transition: all 0.2s; 
            opacity: 0.6;
        }}
        .nav-btn:hover {{ background: var(--accent); color: #000; border-color: var(--accent); opacity: 1; }}
        
    </style>
</head>
<body>
    <div id="sidebar">
        <div class="sidebar-header">
            <span>Grok Viewer</span>
            <div id="search-toggle" onclick="toggleSearchMode()" title="検索モード">&#128269;</div>
        </div>
        
        <!-- 通常モード -->
        <div id="normal-sidebar-content">
            <div class="filter-tabs">
                <div class="filter-tab active" onclick="setFilter('all')">All</div>
                <div class="filter-tab" onclick="setFilter('fav')">Favorites</div>
            </div>
            <ul id="date-list"></ul>
        </div>

        <!-- 検索モード -->
        <div id="search-sidebar-content" class="hidden">
            <div class="search-box">
                <input type="text" id="search-input" placeholder="プロンプトを検索..." onkeydown="if(event.key==='Enter') performSearch()">
                <button class="search-btn" onclick="performSearch()">&#128269;</button>
            </div>
            <div id="search-status">キーワードを入力して検索</div>
            <button class="back-btn" onclick="toggleSearchMode()">← 日付リストに戻る</button>
        </div>
        
        <!-- ナビゲーションボタン -->
        <div id="nav-buttons">
            <button class="nav-btn" onclick="scrollToTop()" title="一番上へ">▲</button>
            <button class="nav-btn" onclick="scrollToBottom()" title="一番下へ">▼</button>
            <button class="nav-btn" onclick="scrollGroup('prev')" title="前のプロンプト">↑</button>
            <button class="nav-btn" onclick="scrollGroup('next')" title="次のプロンプト">↓</button>
        </div>
    </div>

    <div id="main">
        <div id="content-area"></div>
    </div>
    
    <div id="modal" onclick="closeModal(event)">
        <div class="close-hint" onclick="closeModal(event)">Click outside or press Esc to close</div>
        <button class="modal-nav modal-prev" onclick="navigateModal(-1, event)">&#10094;</button>
        <button class="modal-nav modal-next" onclick="navigateModal(1, event)">&#10095;</button>
        <div id="modal-content" onclick="event.stopPropagation()"></div>
    </div>
<script>
    const data = {json_data};
    const dates = {dates_json};
    let currentFilter = 'all'; 
    let currentDate = null;
    let isSearchMode = false;
    
    let currentMediaList = [];
    let currentMediaIndex = -1;
    
    window.onload = () => {{ renderDateList(); if (dates.length > 0) selectDate(dates[0]); }};
    
    document.addEventListener('keydown', (e) => {{ 
        if (document.getElementById('modal').classList.contains('active')) {{
            if (e.key === 'Escape') closeModal();
            if (e.key === 'ArrowLeft') navigateModal(-1);
            if (e.key === 'ArrowRight') navigateModal(1);
        }}
    }});

    function toggleSearchMode() {{
        isSearchMode = !isSearchMode;
        const normalContent = document.getElementById('normal-sidebar-content');
        const searchContent = document.getElementById('search-sidebar-content');
        
        if (isSearchMode) {{
            normalContent.classList.add('hidden');
            searchContent.classList.remove('hidden');
            document.getElementById('search-input').focus();
        }} else {{
            searchContent.classList.add('hidden');
            normalContent.classList.remove('hidden');
            // 元の表示に戻す
            if (currentDate) selectDate(currentDate);
        }}
    }}

    function performSearch() {{
        const query = document.getElementById('search-input').value.trim().toLowerCase();
        if (!query) return;

        const container = document.getElementById('content-area');
        container.innerHTML = '';
        currentMediaList = [];
        
        let hitCount = 0;
        const fragment = document.createDocumentFragment();

        // 全日付を走査
        dates.forEach(date => {{
            const groups = data[date];
            groups.forEach(group => {{
                // プロンプトがある場合のみ検索対象
                if (group.prompt) {{
                    const content = group.prompt.content.toLowerCase();
                    if (content.includes(query)) {{
                        hitCount++;
                        // ヒットしたグループを描画 (selectDateのロジックを流用)
                        renderGroup(group, fragment, date);
                    }}
                }}
            }});
        }});

        if (hitCount === 0) {{
            container.innerHTML = '<div style="text-align:center; padding:50px; color:#888;">No matches found.</div>';
            document.getElementById('search-status').innerText = '0 件ヒット';
        }} else {{
            container.appendChild(fragment);
            document.getElementById('search-status').innerText = `${{hitCount}} 件ヒット`;
        }}
        
        document.getElementById('main').scrollTop = 0;
    }}

    // グループ描画用の共通関数
    function renderGroup(group, parentElement, dateStrLabel) {{
        // 画像リストへの追加
        group.media.forEach(m => currentMediaList.push(m));
        
        // 描画開始時のメディアインデックス
        let baseIndex = currentMediaList.length - group.media.length;

        const groupDiv = document.createElement('div');
        groupDiv.className = 'group';
        
        const promptDiv = document.createElement('div');
        promptDiv.className = 'prompt-header';
        
        if (group.prompt) {{
            const p = group.prompt;
            const dateStr = new Date(p.time * 1000).toLocaleString();
            let label = dateStr;
            // 検索モード時は日付も表示したいのでラベルに追加
            if (isSearchMode && dateStrLabel) label = `[${{dateStrLabel}}] ${{dateStr}}`;
            
            let safeText = p.content.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
            safeText = safeText.replace(/ -/g, '<br><span style="color:#aaa;"> -</span>').replace(/# /g, '<br><span style="color:#81d4fa;"># </span>');
            promptDiv.innerHTML = `<div class="prompt-meta">${{label}}</div><div class="prompt-text">${{safeText}}</div>`;
        }} else {{
            promptDiv.innerHTML = `<div class="no-prompt">No Prompt Info (画像のみ)</div>`;
        }}
        groupDiv.appendChild(promptDiv);

        const gridDiv = document.createElement('div');
        gridDiv.className = 'media-grid';
        
        group.media.forEach((m, i) => {{
            const globalIndex = baseIndex + i;
            const itemDiv = document.createElement('div');
            itemDiv.className = 'media-item';
            itemDiv.title = m.name;
            
            itemDiv.draggable = true;
            itemDiv.ondragstart = (e) => handleDragStart(e, m.path, m.name);

            let iconHtml = '';
            if (m.type === 'video') iconHtml += '<div class="video-label">VIDEO</div>';
            
            if (m.type === 'video') {{
                itemDiv.innerHTML = `<video src="${{m.path}}#t=0.1" muted></video>${{iconHtml}}`;
                itemDiv.onclick = () => openModalByIndex(globalIndex);
            }} else {{
                itemDiv.innerHTML = `<img src="${{m.path}}" loading="lazy" alt="">${{iconHtml}}`;
                itemDiv.onclick = () => openModalByIndex(globalIndex);
            }}
            gridDiv.appendChild(itemDiv);
        }});
        groupDiv.appendChild(gridDiv);
        parentElement.appendChild(groupDiv);
    }}

    function setFilter(filter) {{
        currentFilter = filter;
        document.querySelectorAll('.filter-tab').forEach(el => el.classList.remove('active'));
        document.querySelector(`.filter-tab[onclick="setFilter('${{filter}}')"]`).classList.add('active');
        renderDateList();
        if (currentDate) selectDate(currentDate);
    }}

    function handleDragStart(e, path, filename) {{
        const url = new URL(path, window.location.href).href;
        const mimeType = path.toLowerCase().endsWith('.mp4') ? 'video/mp4' : 'image/jpeg';
        e.dataTransfer.setData('DownloadURL', `${{mimeType}}:${{filename}}:${{url}}`);
        e.dataTransfer.setData('text/uri-list', url);
        e.dataTransfer.setData('text/plain', url);
    }}

    function renderDateList() {{
        const list = document.getElementById('date-list');
        list.innerHTML = '';
        let hasVisibleDate = false;
        dates.forEach(date => {{
            let count = 0;
            data[date].forEach(group => {{
                group.media.forEach(m => {{
                    if (currentFilter === 'all' || m.is_favorite) count++;
                }});
            }});
            if (count === 0 && currentFilter === 'fav') return;
            const li = document.createElement('li');
            li.className = 'date-item';
            if (date === currentDate) li.classList.add('active');
            li.onclick = () => selectDate(date);
            li.id = 'date-' + date;
            li.innerHTML = `${{date}} <span class="count">${{count}}</span>`;
            list.appendChild(li);
            hasVisibleDate = true;
        }});
        if (!hasVisibleDate) list.innerHTML = '<li style="padding:15px; color:#888;">No data found.</li>';
    }}

    function selectDate(date) {{
        currentDate = date;
        document.querySelectorAll('.date-item').forEach(el => el.classList.remove('active'));
        const activeItem = document.getElementById('date-' + date);
        if (activeItem) activeItem.classList.add('active');
        const container = document.getElementById('content-area');
        container.innerHTML = '';
        
        currentMediaList = [];

        if (!data[date]) return;
        const groups = data[date];
        const fragment = document.createDocumentFragment();

        groups.forEach(group => {{
            // フィルタリング
            const validMedia = group.media.filter(m => currentFilter === 'all' || m.is_favorite);
            if (validMedia.length === 0) return;
            
            // フィルタリング済みのメディアだけの仮グループを作って描画関数に渡す
            const displayGroup = {{ ...group, media: validMedia }};
            renderGroup(displayGroup, fragment, null);
        }});

        if (currentMediaList.length === 0) container.innerHTML = '<div style="text-align:center; padding:50px; color:#888;">No favorites in this date.</div>';
        else container.appendChild(fragment);
        document.getElementById('main').scrollTop = 0;
    }}

    function openModalByIndex(index) {{
        if (index < 0 || index >= currentMediaList.length) return;
        currentMediaIndex = index;
        const item = currentMediaList[index];
        
        const modal = document.getElementById('modal');
        const content = document.getElementById('modal-content');
        content.innerHTML = '';
        
        if (item.type === 'video') {{
            content.innerHTML = `<video src="${{item.path}}" controls autoplay style="max-width:100%; max-height:90vh;"></video>`;
        }} else {{
            content.innerHTML = `<img src="${{item.path}}" style="max-width:100%; max-height:90vh;">`;
        }}
        modal.classList.add('active');
    }}

    function navigateModal(direction, event) {{
        if (event) event.stopPropagation();
        const newIndex = currentMediaIndex + direction;
        if (newIndex >= 0 && newIndex < currentMediaList.length) {{
            openModalByIndex(newIndex);
        }}
    }}

    function closeModal(event) {{
        if (event && event.target.id !== 'modal' && !event.target.classList.contains('close-hint')) return;
        document.getElementById('modal').classList.remove('active');
        document.getElementById('modal-content').innerHTML = '';
        currentMediaIndex = -1;
    }}

    // Navigation functions
    function scrollToTop() {{
        document.getElementById('main').scrollTo({{ top: 0, behavior: 'smooth' }});
    }}

    function scrollToBottom() {{
        const main = document.getElementById('main');
        main.scrollTo({{ top: main.scrollHeight, behavior: 'smooth' }});
    }}

    function scrollGroup(direction) {{
        const main = document.getElementById('main');
        const groups = document.querySelectorAll('.group');
        if (!groups.length) return;
        
        const current = main.scrollTop;
        const threshold = 5; 

        if (direction === 'next') {{
            for (const g of groups) {{
                if (g.offsetTop > current + threshold) {{
                    g.scrollIntoView({{ behavior: 'smooth', block: 'start' }});
                    return;
                }}
            }}
        }} else {{
            for (let i = groups.length - 1; i >= 0; i--) {{
                if (groups[i].offsetTop < current - threshold) {{
                    groups[i].scrollIntoView({{ behavior: 'smooth', block: 'start' }});
                    return;
                }}
            }}
            scrollToTop();
        }}
    }}
</script></body></html>"""

    try:
        with open(VIEWER_PATH, "w", encoding="utf-8") as f:
            f.write(html_content)
        print(f"   ✅ 生成完了: {VIEWER_PATH}")
        webbrowser.open(f"file://{VIEWER_PATH}")
    except Exception as e:
        print(f"   ❌ 生成失敗: {e}")
        # エラー時は確認のため待機
        print("\n[Enter]キーを押して終了してください...")
        input()

def main():
    print("=" * 60)
    print(" 🧹 Grok Organizer & Timeline Generator (v2.0)")
    print("=" * 60)
    try:
        video_count = move_videos()
        image_count = clean_garbage_images() 
        organize_prompts()
        fav_set = organize_favorites()
        generate_viewer_html(fav_set)
        print("-" * 60)
        print(f"✨ 完了")
    except Exception as e:
        print(f"\n❌ エラー: {e}")
        print("\n[Enter]キーを押して終了してください...")
        input()

if __name__ == "__main__":
    main()