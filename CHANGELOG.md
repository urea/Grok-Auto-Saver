...
- **プロンプト保存機能の復旧:** Grok側のUI変更（`z-index` 用クラスの `z-10` から `z-20` への変更など）に伴い、拡張機能がプロンプトテキストを抽出できなくなっていた問題を修正。セレクタをより堅牢な本質的クラスのみ（`bg-surface-l1.truncate.rounded-full.sticky`）に依存するよう改善。

## [3.9] - 2026-02-08

### 追加 (Added)

- **一括コピー機能 (Organizer):** ビューアー (`Grok_Viewer.html`) にて、Ctrlキーで複数ファイルを選択した状態でコピーボタンを押すと、選択した全ファイルのパスが改行区切りでクリップボードにコピーされる機能を追加。

### 変更 (Changed)

- **動画再生の挙動改善:** ビューアーにて、Ctrlキーを押しながら動画サムネイルをクリックした際、選択状態の切り替えのみを行い、動画の自動再生（モーダル表示）を抑制するように変更。複数選択時の操作性を向上。

### 修正 (Fixed)

- **依存ライブラリの不足解消:** 画像の解像度チェックに必要な `Pillow` ライブラリが `requirements.txt` に含まれていなかった問題を修正。

## [3.8] - 2026-01-23

### 追加 (Added)

- **スマート・スキップ機能 (Organizer):** 過去フォルダのファイル数に変化がない場合、そのフォルダの再スキャン（解像度チェック・重複チェック）を完全にスキップする機能を実装。数千枚以上の画像がある環境でも、実質的なスキャン対象を「変更のあったフォルダ」のみに限定することで、全検査を数秒で完了できるよう劇的に高速化。

### 変更 (Changed)

- **重複排除の範囲限定化:** 重複チェックを「各日付フォルダ内」に限定するよう仕様変更。これにより、不慮の別日フォルダからの削除を防ぎ、日付を跨いだ意図的な重複保存を許容。

## [3.7] - 2026-01-23

### 変更 (Changed)

- **全検査トリガーの適正化 (Organizer):** 全フォルダのスキャントリガーを「24時間経過」から「日付の切り替わり」に変更。実行中にも関わらず不意に重い処理が始まる問題を解消し、その日最初の実行時に確実にクリーンアップを行う仕様に改善。
- **ユーザーメッセージの刷新:** 全検査実行時およびスキップ時のメッセージを、現在の状況（本日初回かどうか）が伝わりやすい親切な表現に更新。

## [3.6] - 2026-01-22

### 変更 (Changed)

- **リポジトリ構成の標準化:** Python の慣習に従い、`.venv`（仮想環境）および `requirements.txt` をプロジェクトルートに配置。開発環境のセットアップと IDE の自動認識性能を向上。
- **ドキュメント構成の刷新:** 技術仕様書 (Specs) をルートの `/docs` フォルダへ移動して公開可能に。一方で、開発メモや一時ログは `_Dev/docs/misc` に集約して完全非公開化。
- **AI 開発ガイドライン (GEMINI.md) の導入:** AI アシスタントがプロジェクト構造や既存ツールを即座に把握し、効率的な開発を行えるようにするためのローカルガイドラインをルートに設置。

## [3.5] - 2026-01-21

### 追加 (Added)

- **動画ホバー再生:** `<video>` サムネイルにマウスカーソルを合わせると自動で再生され、外すと停止・リセットされる機能を追加。クリックせずに動画内容をプレビュー可能に。

## [3.4] - 2026-01-21

### 追加 (Added)

- **重複画像クリーニング:** ファイル内容(ハッシュ)に基づく重複削除機能を実装。ファイル名が異なっていても中身が同じ画像を削除します（**最も古いファイルを優先して残します**）。
  - ※削除されるファイルがFavoritesで、残るファイルがFavoritesでない場合、**Favoritesステータスを自動的に継承**します。
...
- **コピーボタン機能:**
  - **画像・動画パス:** サムネイル右上および拡大画面に、ファイルパスをコピーするボタンを追加。
  - **プロンプト全文:** プロンプトヘッダー（日付横）に、プロンプトテキスト全文をコピーするボタンを追加。
- **レイアウト改善:** プロンプトの折りたたみ操作を「日付・アイコン部分」のみに限定し、プロンプトテキストの選択（ドラッグ）を容易化。

### 修正 (Fixed)

- **VIDEOラベル表示:** 縦長に伸びてしまうレイアウト崩れを修正。
- **プロンプトコピー:** プロンプトコピー時にファイルパスが含まれてしまう不具合を修正。

## [3.2] - 2026-01-20

### 修正 (Fixed)

- **スキャン対象の適正化:** 検査対象を「当日フォルダのみ」に限定していた仕様ではチェック漏れが発生する問題を修正。過去フォルダも定期的に（24時間に1回）全スキャンすることで、確実なクリーニングと実行速度の両立を実現。

### 変更 (Changed)

- **Organizer:** 実行時に `_Data/System/organizer_state.json` を生成し、前回の全スキャン日時を管理するように変更。
